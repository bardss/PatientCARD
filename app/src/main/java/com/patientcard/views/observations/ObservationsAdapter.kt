...
class ObservationsAdapter(val context: Context?) : RecyclerView.Adapter<ObservationsAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (observationsList?.get(position)?.dateTime != null) {
            holder.observationDateTextView.text = ResUtil.getString(R.string.observation) + " " + FormatTimeDateUtil.getFormattedDateTime(getListItem(position)?.dateTime!!)
        }
        holder.editImageView.setOnClickListener {
            if (context != null && context is ObservationsView) {
                context.clickEditObservation(getListItem(position))
            }
        }
    }

    fun getListItem(position: Int): ObservationDTO? {
